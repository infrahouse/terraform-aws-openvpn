...
module "userdata" {
  ...
  # WIF files (cred config, wif.env, verify-wif.sh) from google-wif.tf.
  ...
  custom_facts = merge(
    var.puppet_custom_facts,
    {
      openvpn : {
        ...
        # Schedules the Google user-revocation sync (puppet-code gates its cron
        # on this fact). Unconditionally true: the WIF feature is always on.
        google_directory_revocation : true
      }
    },
    {
      "efs" : {
...
resource "aws_autoscaling_group" "openvpn" {
  ...
  # statement in iam.tf, or the instance is excluded from Inspector forever.
  tag {
